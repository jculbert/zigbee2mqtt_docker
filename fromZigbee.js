    ...
    zigmate_wind: {
        ...
        convert: (model, msg, publish, options, meta) => {
            if (msg.endpoint.ID == 2) {
                const wind = parseFloat(msg.data['presentValue']);
                return {wind: calibrateAndPrecisionRoundOptions(wind, options, 'wind')};
            }
        },
    },
    zigmate_gust: {
        ...
        convert: (model, msg, publish, options, meta) => {
            if (msg.endpoint.ID == 3) {
                const gust = parseFloat(msg.data['presentValue']);
                return {gust: calibrateAndPrecisionRoundOptions(gust, options, 'gust')};
            }
        },
    },
};

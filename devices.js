...
const devices = [
    ...
    },
    {
        ...
        fromZigbee: [
            fz.on_off_xiaomi_ignore_endpoint_4_5_6, fz.QBKG04LM_QBKG11LM_click, fz.xiaomi_power_from_basic,
            fz.QBKG04LM_QBKG11LM_operation_mode, fz.QBKG11LM_click, fz.ignore_multistate_report, fz.xiaomi_power,
        ],
        toZigbee: [tz.on_off, tz.xiaomi_switch_operation_mode],
        endpoint: (device) => {
            return {'system': 1};
    ...
    },
    {
        ...
        fromZigbee: [
            fz.on_off_xiaomi_ignore_endpoint_4_5_6, fz.QBKG03LM_QBKG12LM_click, fz.xiaomi_power_from_basic,
   ...
   {
        zigbeeModel: ['zm.weather'], // The model ID from: Device with modelID 'lumi.sens' is not supported.
        model: 'zmweather', // Vendor model number, look on the device for a model number
        vendor: 'ZigMate', // Vendor of the device (only used for documentation and startup logging)
        description: 'ZigMate Weather Station', // Description of the device, copy from vendor site. (only used for documentation and startup logging)
        supports: 'temperature, wind, and gust', // Actions this device supports (only used for documentation)
        fromZigbee: [fz.battery, fz.xiaomi_temperature, fz.zigmate_wind, fz.zigmate_gust],
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            endpoint = device.getEndpoint(2);
            await bind(endpoint, coordinatorEndpoint, [
                'genAnalogInput',
            ]);
            endpoint = device.getEndpoint(3);
            await bind(endpoint, coordinatorEndpoint, [
                'genAnalogInput',
            ]);
        }
    ...
    {
        zigbeeModel: ['cb.relay8'],
        model: 'cb.relay8',
        vendor: 'Custom devices (DiY)',
        description: '8 relay switch',
        supports: 'on/off',
        fromZigbee: [fz.on_off],
        toZigbee: [tz.on_off],
        endpoint: (device) => {
            return {'l1': 2, 'l2': 3, 'l3': 4, 'l4': 5, 'l5': 6, 'l6': 7, 'l7': 8, 'l8': 9};
        },
        meta: {configureKey: 2, multiEndpoint: true},
        configure: async (device, coordinatorEndpoint) => {
            await bind(device.getEndpoint(2), coordinatorEndpoint, ['genOnOff']);
